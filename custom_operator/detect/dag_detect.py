...
with open(
    DEFAULT_PATH + "/dag_description/dag_detect/DAG-Detect_data_by_using_userInput.md",
    "r",
) as file:
    DESCRIPTION = file.read()

test_obj = UserDetectInput(
    detection_target={"s_IP": "10.44.44.*"}, frequency=3, interval=3
)
...
with DAG(
    ...
    detect_target_task = DetectSensorOperator(
        task_id="Detect_target_", conn_id=CONNECTION, input=test_obj
    )
