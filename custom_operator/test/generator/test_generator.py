...
def test_AddSampleLogOperator():
    test_obj = AddSampleLogOperator(
        index_name="ips", size=10000, conn_id="docker_elastic"
    )
...
def test_ConvertDataFrame():
    ...
    mockup_obj = AddSampleLogOperator(
        index_name="ips", size=10000, conn_id="docker_elastic"
    )
...
def test_MakeRandomData():
    ...
    test_value = {
        "UNKNOWN2": bytearray(b"q\x921\x07kb\xd4\x95SE"),
        "SID": "2017143",
        "UNKNOWN3": bytearray(b"!|\xfb\x03\xd9@<\xdd\xfb\x9a"),
        "@version": "1",
        "tags": ["_grokparsefailure"],
        "UNKNOWN1": bytearray(b"TD\x15\xf0\xbf\x1f\x94 \x88("),
        "GID": "1",
        "KUNKNOWN4": "7",
        "d_port": "52025",
        "tag": "ips",
        "snort": "snort",
        "descriptoin": "(http_inspect) NO CONTENT-LENGTH OR TRANSFER-ENCODING IN HTTP RESPONSE",
        "class": " Not Suspicious Traffic",
        "priority": "2",
        "protocol": "TCP",
        "event": {
            "original": "<9>1 2022-09-21T02:28:19.429156+09:00 pfSense.localdomain snort 66091 - - [1:2013504:3] ET POLICY GNU/Linux APT User-Agent Outbound likely related to package management [Classification: Not Suspicious Traffic] [Priority: 3] {TCP} 10.0.0.254:31686 -> 91.189.91.38:80"
        },
