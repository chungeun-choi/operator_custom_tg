...
class MakeIndexOperator(BaseOperator):
    ...
    def __init__(self, conn_id, index_name, **kwargs) -> None:
        ...
        self._index_template_path = (
            DEFALUT_PATH + "index_template/{}_template.json".format(index_name)
        )
        self._index_ilm_path = DEFALUT_PATH + "ilm_template/{}_ilm.json".format(
            index_name
        )
    ...
    def make_index_template(self):
        ...
        with open(self._index_template_path, "r") as file:
            ...
            print(template_data)
            self.conn.indices.put_index_template(
                name="{}_tempalte".format(self._index_name), body=template_data
            )
