    ...
    def execute(self, context: Context):
        ...
        self._created_datas = GenerateLogDatas(
            ...
            or datetime.fromtimestamp(
                context["execution_date"].timestamp(), pendulum.tz.UTC
            ),
            end_date=self._end_date
            or datetime.fromtimestamp(
                context["next_execution_date"].timestamp(), pendulum.tz.UTC
            ),
        ).generate()
        data = [
            {"_index": self._index_name, "_id": create_id(19), "_source": data}
            for data in self._created_datas
        ]
    ...
    def __init__(self, db: int = None):
        try:
            ...
            self.conn_info = Connection(
                conn_id="redis", host="operator_custom_tg-redis-1", port=6379
            )
    ...
    def extract_case(self):
        ...
        for filed in fileds:
            if filed in (
                ...
                "d_IP",
                "@timestamp",
            ) or filed.startswith("UNKNOWN"):
                continue
            else:
                case_data.setdefault(
                    filed, self._convert_data[filed].drop_duplicates().to_list()
                )
...
class MakeRandomData:
    def __init__(self, index_name: str, start_date: datetime, end_date: datetime):
        ...
        self._end_date = end_date
        self._redis_sample = (
            RedisHook(1).get_conn().get("sample_{}".format(self._index_name))
        )
    ...
    def _make_radom_IP(self):
        if (
            "s_IP" in self._redis_sample["key_list"]
            and "d_IP" in self._redis_sample["key_list"]
        ):
            ...
            if field.startswith("UNKNOWN"):
                ...
                try:
                    random_value = np.random.choice(
                        self._redis_sample["case_data"][field]
                    )
