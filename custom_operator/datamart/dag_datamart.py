...
with DAG(
    ...
    params={
        ...
        "query": Param("SELECT s_IP,count(*) FROM iis_log GROUP BY s_IP HAVING count(*) > 10", type=["array", "string"]),
    ...
    make_datamart_task = MakeDataMartOperator(
        task_id ="make_data_mart_task",
        input = UserInput(**dag.params),
        conn_id=CONNECTION
    )
    check_save_type = BranchPythonOperator(
            task_id='check_save_type',
